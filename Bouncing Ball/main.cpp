...
void DrawWall(int rowSize, int colSize, int isVertical) {
	for (double i = 0; i < rowSize; i++) {
		int modZ = (int)(i) % 2;
		for (double j = 0; j < colSize; j++) {
			...
			DrawCube(new double[3]{ i,0,j }, new double[4]{ 0,0,0,0 }, new double[3]{ 1,1,1 }, color);
		}
	}
}
...
void Display(void)
{
	...
	outputString.str("");
	outputString << "Left steps: " << leftSteps;
	printOnScreen(winX * 0.1, winY * 0.05, outputString.str());

	outputString.str("");
	outputString << "Right steps: " << rightSteps;
	printOnScreen(winX * 0.3, winY * 0.05, outputString.str());

	outputString.str("");
	outputString << "Up steps: " << upSteps;
	printOnScreen(winX * 0.6, winY * 0.05, outputString.str());

	outputString.str("");
	outputString << "Down steps: " << downSteps;
	printOnScreen(winX * 0.8, winY * 0.05, outputString.str());
